...
int *sampleValues(void *arguments) {
    ...
    if ((!r->lchild) && (!r->rchild)) {
        ...
        double lElems = elemsIntersection(&(r->lchild->filter), mainFilter, r->lchild->nOnes, noOfSampledElements);
        double rElems = elemsIntersection(&(r->rchild->filter), mainFilter, r->rchild->nOnes, noOfSampledElements);
...
int *multipleSamples(int noOfSampledElements) {
    double lElems = elemsIntersection(&(rooted->left->filter), mainFilter, rooted->left->nOnes, noOfSampledElements);
    double rElems = elemsIntersection(&(rooted->right->filter), mainFilter, rooted->right->nOnes, noOfSampledElements);
...
int main(int argc, char *argv[]) {
    ...
    int *result = multipleSamples(numSamples);
