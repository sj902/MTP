/**This file contains necessary utility functions for BloomTree
 ...
 * Questions:
 * 1. What is k1
 * 2. Why do we need the tree structure/ Can't we just do away with the node?
 * 3. Why are we hard-coding Vect_Size in bloom.c?
 * **/
...
extern struct ThreadInfo *threadInfRoot;
int levelThreshold;
int T;
int nMembership = 0, numIntersections = 0;
extern int needThreads;
struct bloomNode {
    struct bloom filter;
...
printf("%d ",count_ones(r->lchild->filter.bloom_vector[i]) );
}
printf("\n");
for (int i=0;i<((VECT_SIZE/NUM_BITS)+1);i++){
printf("%d ",count_ones(r->rchild->filter.bloom_vector[i]) );
}
printf("\n");
...
int sampleFromTree(struct bloom *k, struct bloomTree *a, int k1) {
	...
	//rElems=.5;
for (int i=0;i<((VECT_SIZE/NUM_BITS)+1);i++){
printf("%d ",count_ones(a->left->filter.bloom_vector[i]) );
}
printf("\n");
for (int i=0;i<((VECT_SIZE/NUM_BITS)+1);i++){
printf("%d ",count_ones(a->right->filter.bloom_vector[i]) );
}
printf("\n");
